...
describe("<BalancesSection /> — liquidación", () => {
    it("muestra las transferencias pendientes y el balance del usuario", async () => {
        ...
        render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(BalancesSection, { groupId: GROUP_ID, currentUserId: ME, currentUserName: "Yo", members: members }) }));
        ...
        expect(screen.getByRole("button", { name: /Saldar/i })).toBeInTheDocument();
    });
    ...
    it("saldar una transferencia la quita de la lista y muestra la liquidación", async () => {
        ...
        // recién creado.
        let balancesState = balancesWithTransfer;
        let settlementsState = [];
        fetchMock.mockImplementation((input) => {
            const url = typeof input === "string" ? input : input.url;
        ...
        // eslint-disable-next-line no-console
        console.log("[test] about to click Saldar, disabled:", settleButton.disabled);
        ...
        // primer test.
        expect(await screen.findByText(/saldó/i, undefined, { timeout: 3000 })).toBeInTheDocument();
        // Y a que la lista de transferencias pendientes esté vacía.
        expect(await screen.findByText(/No hay deudas pendientes/i, undefined, {
            timeout: 3000,
        })).toBeInTheDocument();
