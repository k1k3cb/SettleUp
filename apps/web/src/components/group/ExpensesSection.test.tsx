...
function mockListResponse(expenses: ExpenseWithSplits[] = []) {
  return {
    ok: true,
    status: 200,
...
function mockCreateResponse(expense: ExpenseWithSplits) {
  return {
    ok: true,
    status: 201,
...
describe("<ExpensesSection /> — lista", () => {
  it("muestra el estado vacío cuando no hay apuntes", async () => {
    ...
    expect(
      await screen.findByText(/Anota el primer gasto de la cuenta/i),
    ).toBeInTheDocument();
...
describe("<ExpensesSection /> — flujo de crear gasto", () => {
  it("anota un gasto a partes iguales y lo muestra en la lista", async () => {
    ...
    const createdExpense: ExpenseWithSplits = {
      id: "e-1",
      groupId: GROUP_ID,
